...
### Сдаю Домашнее задание "Разделение доступа в DRF".

### Из дополнительных заданий попробовал реализовать:
### -Права для админов
### -Модерация. (это не доп.задание, это так сказать, попытка глубже понять что происходит под капотом)) Под модерацией я подразумевал проверку администратором объявления, созданного аут.пользователем. Только после того как администратор поставит в админке галочку Open (или пошлет PUT запрос) объявления можно просматривать.

### Для первого задания я сделал классы ForAny,IsOwner,IsAdmin которые мною использовались в зависимости от http_method пользователя.
### По второй задачи я долго думал в какой момент должно происходить проверка на предмет возможности просмотра пользователями. Прочитал про to_representation и попробовал его переопределить в классе сериализатора.

### Проверьте пожалуйста, жду ваших замечаний. 
